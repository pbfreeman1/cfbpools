...
- **Weekly Pick'em Pool** (build second, spec still evolving): pick 6 games against the spread each week. 6-0 wins the pot; pushes count as losses; unlimited entries per user; picks lock 12pm Saturday.
  - **`get_pickem_entry_picks(p_entry_id)` hides not-yet-kicked-off picks ONLY when the viewer isn't the entry's owner.** The RPC's WHERE clause is `g.kickoff_time <= now() OR EXISTS (pickem_entries e WHERE e.id = p_entry_id AND e.user_id = auth.uid())` — a user always sees every pick on their own entry (leaderboard expand row, homepage "your entries", edit page). RLS on `pickem_picks` already permits this (its SELECT policy has an owner clause alongside the kickoff clause), so the RPC is the only enforcement point for the privacy rule. `ExpandablePicks.tsx` takes an `isOwn` prop purely to word the empty state; the leaderboard passes `row.isOwn`, `OwnEntryCard` hardcodes it.
  - **The `pickem_entries_log` / `pickem_picks_log` triggers re-log a row on EVERY update to the source table**, including the grading job writing `result`/`is_final` onto each pick — so a bare `version > 1` count is mostly grading noise, not user edits. To count/show real edits (admin entries list "N edits" badge + the per-entry Audit trail on `/admin/pickem/entries/[entryId]`), walk each key's log rows in version order and only treat a row as a change when its value differs from the previous version (`team_id` per `(entry_id, game_id)` for picks, `entry_name` for the entry). Admin pick *clears* are `DELETE`s and never hit `pickem_picks_log` — those live in `admin_actions` only.
  - **`/admin/pickem/entries` takes the same `?schedule_id=` week override** as `/pickem/leaderboard` and `/pickem/entries/new` (`params.schedule_id || app_settings.current_week_id`), with a week `<select>` matching `/admin/pickem/week`'s. The override is threaded through the page's own links (search form, Clear, per-row Edit, and the detail page's "← Entries" back link) via a `withWeek()` helper / `entriesHref`; server-action redirects still land on the bare current-week list.
- **The Survivor pick page (`/survivor/[entryId]`) is a single-week view, not a stacked list.** `page.tsx` (server) fetches every week's data up front and hands it to `PickTool.tsx` (client), which holds the selected week in `useState` and renders only that week. Switching weeks via the horizontal week-selector strip is pure client state — the URL's `?week=` is kept in sync with `window.history.replaceState`, deliberately *not* the Next.js router, so it never triggers a server round-trip. `?week=N` is still the real deep-link contract: the home entry grid's per-week links and `savePick()`'s redirects (`app/actions/survivor.ts`) all read/write it, so a fresh load or a save-and-reload lands back on the same week. If this page changes again, keep switching weeks client-only — routing through `router.push`/`replace` here would re-run the server component on every strip click.
  ...
  - **The live bonus-pick flow is `/survivor/entries/[id]/bonus` (week selector) + `/survivor/entries/[id]/bonus/[week]` (`BonusTeamSelect.tsx`).** The old `/survivor/[entryId]/bonus` route is now just a redirect to it. An **unlocked** existing bonus week is editable in place — the `[week]` page passes `existingRegularTeamId` (= `team_a_id`, the regular slot) and `existingBonusTeamId` (= `team_b_id`) into `BonusTeamSelect`, which pre-selects both — or clearable via a "Clear bonus pick" button that `formAction`s `clearBonusPick()` (`app/actions/survivor-bonus.ts`). Clearing `DELETE`s the `survivor_bonus_picks` row; the `unsync_survivor_bonus_pick_from_picks` AFTER DELETE trigger flips the synced `survivor_picks` row back to a plain regular pick (`is_bonus_week=false`, `bonus_team_id=null`). **Lock** = earliest kickoff among games involving either of the pick's two teams `<= now` — computed the same way in three places that must agree: `validate_survivor_bonus_pick_delete` / `validate_survivor_bonus_pick` (UPDATE branch) server-side, the `[week]` page (renders a read-only summary, no edit/clear controls), and `bonus/page.tsx` (`usedLocked` → shows an "Edit" link only when unlocked). The per-week cap check on the `[week]` page counts bonus picks in *other* weeks only (`.neq("schedule_id", …)`) so an existing bonus week stays editable once both slots are spent — matching the trigger's `id is distinct from NEW.id`. RLS: `survivor_bonus_picks_delete` allows the entry owner (replaced the admin-only delete policy).
